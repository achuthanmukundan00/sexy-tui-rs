...
/// Decode a Kitty protocol printable key (CSI u format with shifted codepoint).
pub fn decode_kitty_printable(data: &str) -> Option<char> {
    // Match: ESC [ <codepoint> u  or  ESC [ <codepoint> ; <modifier> u
    let body = data.strip_prefix("\x1b[")?;
    let body = body.strip_suffix('u')?;
    let codepoint_str = body.split(';').next()?;
...
pub fn matches_key(data: &str, key_id: &str) -> bool {
    ...
    match parsed.key.as_str() {
        ...
        "enter" | "return" => {
            if modifier == 0 && data == "\r" { return true; }
            if modifier == modifiers::ALT && data == "\x1b\r" { return true; }
            matches_kitty_sequence(data, codepoints::ENTER, modifier)
                || matches_modify_other_keys(data, codepoints::ENTER, modifier)
        }

        "tab" => {
            if modifier == 0 && data == "\t" { return true; }
            if modifier == modifiers::SHIFT && data == "\x1b[Z" { return true; }
        ...
        "backspace" => {
            if modifier == 0 {
                ...
                if data == "\x08" { return true; } // Windows Terminal
            }
        ...
        // Function keys
        fk @ "f1" | fk @ "f2" | fk @ "f3" | fk @ "f4" | fk @ "f5" | fk @ "f6"
        | fk @ "f7" | fk @ "f8" | fk @ "f9" | fk @ "f10" | fk @ "f11" | fk @ "f12" => {
            let f_code = match fk {
                "f1" => codepoints::F1, "f2" => codepoints::F2, "f3" => codepoints::F3,
                "f4" => codepoints::F4, "f5" => codepoints::F5, "f6" => codepoints::F6,
                "f7" => codepoints::F7, "f8" => codepoints::F8, "f9" => codepoints::F9,
                "f10" => codepoints::F10, "f11" => codepoints::F11, "f12" => codepoints::F12,
                _ => return false,
            };
