...
impl CombinedAutocompleteProvider {
    pub fn new(commands: Vec<SlashCommand>, base_path: String) -> Self {
        CombinedAutocompleteProvider { commands, base_path }
    }

    fn complete_slash_commands(&self, query: &str) -> Vec<AutocompleteItem> {
        let cmd_refs: Vec<&SlashCommand> = self.commands.iter().collect();
        let filtered: Vec<&SlashCommand> = fuzzy_filter(&cmd_refs, query, |cmd: &&SlashCommand| cmd.name.clone());
        filtered.into_iter().map(|cmd| {
            AutocompleteItem {
